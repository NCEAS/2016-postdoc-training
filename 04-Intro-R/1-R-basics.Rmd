...
Big fan of keyboard shortcuts? Check out _Tools → Keyboard Shortcuts Help_.
...
You construct a data frame using `data.frame()`:
...
To construct a path to a file, it is recommended to use `file.path` and not `paste` as it will take care of the dfifferent path convention bewteen OS and it is faster.
...
Note: by default these function will transform your column type as factors (variables in R which take on a limited number of different values). This often something you do not want. You can specify this by adding the option `stringsAsFactors = FALSE`:
